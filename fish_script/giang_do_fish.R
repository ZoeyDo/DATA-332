library(ggplot2)
library(dplyr)
library(tidyverse)
library(readxl)
library(here) 
library(kableExtra)
library(readr)


setwd('~/Documents/DATA332/r_projects/fish')

kelp_fronds <- read_excel('kelp_fronds.xlsx', .name_repair = 'universal')
fish <- read_csv("fish.csv")

fish_garibaldi <- fish %>%
  dplyr::filter(common_name == "garibaldi")

fish_mohk <- fish %>%
  dplyr::filter(site == "mohk")

fish_over50 <- fish %>%
  dplyr::filter(total_count >= 50)

fish_3sp <- fish %>%
  filter(common_name == "garibaldi" |
           common_name == "blacksmith" |
           common_name == "black surfperch"
         )

fish_3sp <- fish %>% 
  filter(common_name %in% c("garibaldi", "blacksmith", "black surfperch"))

fish_gar_2016 <- fish %>% 
  filter(year == 2016 | common_name == "garibaldi")

aque_2018 <- fish %>% 
  filter(year == 2018, site == "aque")

aque_2018

# Written as sequential filter steps:

aque_2018 <- fish %>% 
  filter(year == 2018) %>% 
  filter(site == "aque")

low_gb_wr <- fish %>% 
  filter(common_name %in% c("garibaldi", "rock wrasse"), 
         total_count <= 10)

#str_detect() is a function from the stringr package that checks if a string contains a given pattern.
fish_bl <- fish %>% 
  filter(str_detect(common_name, pattern = "black"))

fish_it <- fish %>% 
  filter(str_detect(common_name, pattern = "it"))

abur_kelp_fish <- kelp_fronds %>% 
  full_join(fish, by = c("year", "site")) 

kelp_fish_left <- kelp_fronds %>% 
  left_join(fish, by = c("year","site"))

kelp_fish_injoin <- kelp_fronds %>% 
  inner_join(fish, by = c("year", "site"))

#The mutate() function is used to create new columns or modify existing columns in a data frame.
my_fish_join <- fish %>% 
  filter(year == 2017, site == "abur") %>% 
  left_join(kelp_fronds, by = c("year", "site")) %>% 
  mutate(fish_per_frond = total_count / total_fronds)

kable(my_fish_join)

my_fish_join %>% 
  kable() %>% 
  kable_styling(bootstrap_options = "striped", 
                full_width = FALSE)
